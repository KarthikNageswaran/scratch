 ...
 *
 * The bacteria only double every forty minutes and we aren't
 * given any more detailed information. So we find that the 
 * size of the colony is defined by:
 *
 ...
 *           ...|...
 *
 * Formula to determine time it takes for colony of two bacteria 
 * to reach 1,000,000 based on doubling in size every twenty 
 * minutes is:
 *
...
{
  const double doublingTime = 20.0;
  const double patientDies = 1000000; /* 1 million */
  const double timeWhenPatientDies = (
    doublingTime * (log(patientDies/2) / log(2))
  ); /* THIS IS THE ANSWER */
  const double maxAntidoteAdminMins = (
    2.0 /* hours */ * 60.0 /* minutes per hour */
  );
  const double maxAntidoteAdminTime = (
    timeWhenPatientDies - maxAntidoteAdminMins
  );
  const double maxTimeHours = 
    floor(maxAntidoteAdminTime / 60.0);
  const double maxTimeMinutes = 
    floor(maxAntidoteAdminTime - (maxTimeHours * 60.0));
  const double maxTimeSeconds = 
    floor(
      ((maxAntidoteAdminTime - (maxTimeHours * 60.0)) - maxTimeMinutes) * 60
    );
  
  printf(
    "Minutes until colony reaches 1,000,000: %.2f, which is about %.5f hours\n"
    , timeWhenPatientDies
